import XCTest
@testable import FermentWatch

final class FermentWatchTests: XCTestCase {
    ...
    func test_addEntry_increasesCount() {
        ...
        let entry = Entry(projectName: "Test", date: Date(), note: "note")
    ...
    func test_addEntry_blockedAtFreeLimit() {
        ...
        while store.entries.count < Store.freeLimit {
            _ = store.add(Entry(projectName: "Filler", date: Date(), note: ""), isPro: false)
        }
        let result = store.add(Entry(projectName: "Overflow", date: Date(), note: ""), isPro: false)
    ...
    func test_addEntry_proBypassesLimit() {
        ...
        let result = store.add(Entry(projectName: "ProEntry", date: Date(), note: ""), isPro: true)
    ...
    func test_deleteEntry_removesFromList() {
        let store = Store()
        let entry = Entry(projectName: "ToDelete", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        store.delete(entry)
    ...
    func test_updateEntry_changesFields() {
        let store = Store()
        var entry = Entry(projectName: "Original", date: Date(), note: "")
        _ = store.add(entry, isPro: false)
        entry.projectName = "Updated"
        store.update(entry)
        XCTAssertEqual(store.entries.first(where: { $0.id == entry.id })?.projectName, "Updated")
    }
